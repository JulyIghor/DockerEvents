...
[ -z "${TELEGRAM_API_TOKEN}" ] && {
    printError "TELEGRAM_API_TOKEN is not defined"
    exit 1
}
[ -z "${TELEGRAM_GROUP_ID}" ] && {
...
function docker_event {
    ...
    [[ "${CONTAINER_NAME}" == ${FILTER_NAME} ]] || {
        echo "skipping ${CONTAINER_NAME} since FILTER_NAME not match filter ${FILTER_NAME}"
        return
    }
    [[ "${CONTAINER_IMAGE}" == ${FILTER_IMAGE} ]] || {
        echo "skipping ${CONTAINER_NAME} since CONTAINER_IMAGE (${CONTAINER_IMAGE}) not match filter ${FILTER_IMAGE}"
        return
    }
    ...
    [[ "${RESTART_POLICY}" == ${FILTER_RESTART_POLICY} ]] || {
        echo "skipping ${CONTAINER_NAME} since FILTER_RESTART_POLICY (${RESTART_POLICY}) not match filter ${FILTER_RESTART_POLICY}"
        return
    }
...
'
        ;;
    die)
        [[ "${EVENT_EXITCODE}" == ${FILTER_EXITCODE} ]] || {
            echo "skipping ${CONTAINER_NAME} since EVENT_EXITCODE (${EVENT_EXITCODE}) not match filter ${FILTER_EXITCODE}"
            return
        }
...
'
    ...
    health_status:*)
        ...
        [[ "${EVENT_STATUS}" == ${FILTER_HEALTH} ]] || {
            echo "skipping ${CONTAINER_NAME} since EVENT_STATUS (${EVENT_STATUS}) not match filter ${FILTER_HEALTH}"
            return
        }
        MESSAGE='
...
'
    ...
    *)
    ...
    [ -z "${MESSAGE}" ] && {
        echo "skipping ${CONTAINER_NAME} since MESSAGE is empty"
        return
    }
